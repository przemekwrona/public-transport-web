...
public interface StopRepository extends JpaRepository<StopEntity, String> {
    ...
    @Query("SELECT s FROM AgencyEntity a JOIN a.routeEntities r  JOIN r.tripEntities t JOIN t.tripProfiles p JOIN p.stopTimes st JOIN st.stopEntity s WHERE :agency = a")
